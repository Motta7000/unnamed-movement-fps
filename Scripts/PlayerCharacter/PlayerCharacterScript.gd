		...
		match currentState:
			states.IDLE:
				if event.is_action_pressed("jump"):
					jump(0.0, false)
				elif event.is_action_pressed("crouch | slide"):
					crouchStateChanges()
			...
			states.CROUCH:
				if event.is_action_pressed("run") and !ceilingCheck.is_colliding():
					walkStateChanges()
				elif event.is_action_pressed("crouch | slide") and !ceilingCheck.is_colliding():
					walkStateChanges()
			...
			states.JUMP:
				if event.is_action_pressed("crouch | slide"):
					slideStateChanges()
				elif event.is_action_pressed("dash"):
					dashStateChanges()
				elif event.is_action_pressed("jump"):
...
func applies(delta):
	...
	if is_on_floor() or !is_on_floor():
		...
		if currentState == states.SLIDE:
			if !startSlideInAir and GameManager.lastFramePosition.y < position.y: slideTime = -1 #if character slide on an uphill, cancel slide
			if slopeAngle < 16:
				if slideTime > 0: 
					slideTime -= delta
			if slideTime <= 0: 
...
func move(delta):
	...
	GameManager.lastFramePosition = position
...
func slideStateChanges():
	...
	if inputDirection != Vector2.ZERO and timeBeforeCanSlideAgain <= 0 and GameManager.lastFramePosition.y >= position.y: #character can slide only on flat or downhill surfaces
